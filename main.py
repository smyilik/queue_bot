...
def get_whitelist():
    return [int(i) for i in open('whitelist.txt').readlines()]
...
async def queue_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(
        f'{datetime.now().strftime('%H:%M:%S')} {update.message.from_user.username} join in {update.message.chat.type}')
...
async def whitelist_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.from_user.id == 826871795:
        ...
        new_user = update.message.text.replace(BOT_USERNAME, '').replace('/whitelist_add', '').strip() + '\n'
    ...
    print(
        f'************************\n'
        f'{datetime.now().strftime('%H:%M:%S')} '
        f'{update.message.from_user.username} whitelist_add {new_user} in {update.message.chat.type}\n'
        f'************************')
...
async def whitelist_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.from_user.id == 826871795:
        ...
        with open('whitelist.txt', 'w') as file:
            file.writelines(line for line in lines if line.strip() != user)
    ...
    print(
        f'************************\n'
        f'{datetime.now().strftime('%H:%M:%S')} '
        f'{update.message.from_user.username} whitelist_remove {user} in {update.message.chat.type}\n'
        f'************************')
